...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left::right::root => (makeCodeTree(left, right)::root).sortBy(weight)
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def dfs(node: CodeTree, bs: List[Bit]): List[Char] = {
      node match {
        case Leaf(c, _) => if (bs.isEmpty) List(c) else c::decode(tree, bs)
        case Fork(l, r, _, _) => if (bs.head == 0) dfs(l, bs.tail) else dfs(r, bs.tail)
      }
    }
    dfs(tree, bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def dfs(node: CodeTree, c: Char): List[Bit] = {
      node match {
        case Leaf(_, _) => Nil
        case Fork(l, r, _, _) => if (chars(l) contains c) 0::dfs(l, c) else 1::dfs(r, c)
      }
    }
    text.flatMap(dfs(tree, _))
  }
  ...
  /**
    ...
    */
  def codeBits(table: CodeTable)(c: Char): List[Bit] = {
    table.find(_._1 == c).get._2
  }
